...
var $author$project$Main$initialModel = {index: 0};
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $author$project$Main$noCmd = function (m) {
	...
	[
		...
		$author$project$Main$Article(
		{
			document: _List_fromArray(
				[
					$author$project$Main$Text('このelm製スライド(300行程度)のコンパイル時間'),
					A2(
					$author$project$Main$Pic,
					$elm$core$Maybe$Just(200),
					'slide-compile-time.png')
				]),
		...
		$author$project$Main$Article(
		{
			document: _List_fromArray(
				[
					$author$project$Main$Code('module Page1 exposing(page)\n\npage =\n  { init = ...\n  , update = ...\n  , view = ...\n  }\n            '),
					$author$project$Main$Code('module Main exposing(..)\n\ntype Model\n  = Model1 Page1.Model\n  | Model2 Page2.Model\n\ntype Msg\n  = Msg1 Page1.Msg\n  | Msg2 Page1.Msg\n\ninit = ...\nupdate msg mPrev =\n  case (msg, mPrev) of\n    (Msg1 page1msg, Model1 page1model) ->\n      Page1.page.update page1msg page1model\n    ...\nview = ...\n')
				]),
			title: $elm$core$Maybe$Just('修正箇所(修正前)')
		}),
		$author$project$Main$Article(
		{
			document: _List_fromArray(
				[
					$author$project$Main$Code('module Page1 exposing(init, update, view)\n\ninit = ...\nupdate = ...\nview = ...\n\n            '),
					$author$project$Main$Code('type Model\n  = Page1 Page1.Model\n  | Page2 Page2.Model\n\ntype Msg =\n     Msg1 Page1.Msg\n   | Msg2 Page1.Msg\n\ninit = ...\nupdate msg mPrev =\n  case (msg, mPrev) of\n    (Msg1 page1msg, Model1 page1model) ->\n      Page1.update page1msg page1model\n    ...\nview = ...\n')
				]),
			title: $elm$core$Maybe$Just('修正箇所(修正後)')
		}),
		$author$project$Main$Article(
		{
			document: _List_fromArray(
			...
			title: $elm$core$Maybe$Just('修正後のコンパイル時間')
		}),
		$author$project$Main$TitleOnly(
		{title: '原因の深掘り'}),
		$author$project$Main$TitleOnly(
	...
	function (msg, mPrev) {
		...
		while (true) {
			switch (msg.$) {
				case 'NextPage':
					if (_Utils_cmp(
						$elm$core$List$length($author$project$Main$pages),
						mPrev.index + 1) < 1) {
						return $author$project$Main$noCmd(mPrev);
					} else {
						var $temp$msg = $author$project$Main$SetPage(mPrev.index + 1),
							$temp$mPrev = mPrev;
						msg = $temp$msg;
						mPrev = $temp$mPrev;
						continue update;
...
var $author$project$Main$buttons = function (m) {
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$Events$onClick($author$project$Main$PrevPage)
					]),
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text('prev')
					])),
				$rtfeldman$elm_css$Html$Styled$text(
				$elm$core$String$fromInt(m.index + 1) + ('/' + $elm$core$String$fromInt(
					$elm$core$List$length($author$project$Main$pages)))),
				A2(
				$rtfeldman$elm_css$Html$Styled$button,
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$Events$onClick($author$project$Main$NextPage)
					]),
				_List_fromArray(
					[
...
var $author$project$Main$renderDocument = function (document) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_Nil,
		_List_fromArray(
...
var $author$project$Main$renderPage = function (page) {
	return A2(
		$rtfeldman$elm_css$Html$Styled$div,
		_List_Nil,
		_List_fromArray(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A2(
					$elm$core$Maybe$map,
					$author$project$Main$renderPage,
					A2($elm_community$list_extra$List$Extra$getAt, model.index, $author$project$Main$pages))),
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{
		init: function (_v0) {
			return $author$project$Main$noCmd($author$project$Main$initialModel);
		},
